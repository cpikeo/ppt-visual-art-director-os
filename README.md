...
当前版本 **7.2.0**（7.2 = 七层深审：删除意图层零消费者镜像、ROUTES 死列、recommend_mode
死链、`run --build`、compiler 内置 guard 死路径与三个死/重复 guard 检查；QC 像素判定跨
draft→release 复用（仅 retry 策略按阶段重算）。不新增能力面，详见 `AUDIT.md` 与 CHANGELOG），
只删重复叙述与固定答案，不删事实、字段名与阻断码（228 项回归契约全过）；既有素材在
...
`assets` 默认只打印紧凑摘要；只有显式 `--show-prompts` 才把完整 prompt/negative 打到终端，`--json` 供机器消费完整清单。
...
└── scripts/
    ...
    └── selftest.py           # 最小验证网（228 项，含判断层、端到端生产链、反退化与速度档检查）
```
...
```bash
python scripts/selftest.py        # 228 项：交付链 / 生产链契约 / 契约拦截 / 判断层 / 反退化 / 静默失效缝 / 速度档
```
