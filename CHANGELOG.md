...
grid_exempt 色块未吸附，配对间距被撕开。

**修复**
- guard `optical_alignment` 扩展：除垂直居中外，校验色块-文字水平间距舒适区（4–20px），
  且同页各对应组间距差 ≤2px，否则 warn（「间距一致」是图例/读数对的硬契约）。
- 经验：成对光学元素要么双双 grid_exempt、要么双双落在网格上——混用会被吸附撕开间距。

## v4.24 细节根因批（2026-09-15）：投影根除、光学对齐、保边吸附、图例位置对应

**案源**：高端茶路演 deck 评审红框：图例色块与文字错位、柱底越过基线、堆叠条图例与分段位置不对应；廉价投影外溢。

**修复（均为根因级，不扩模板）**
- compiler：每页形状 `shadow.inherit=False`，保存后从 theme XML 移除 `outerShdw`
  （部分渲染器无视 per-shape 覆盖）；深度感只靠色阶/留白/细线。
- compiler stacked_bar：图例项居中于各自分段下方，分段过窄放不下时回落均布。
- guard normalizer：shape 类元素尺寸改为「远边就近吸附」——与基线/邻块共享的边保持精确
  （ceil「只增不减」会破坏共边，柱底压线类缺陷的根因）；文本/容器仍只增不减防溢出。
- guard 新静态规则：`optical_alignment`（小色块与右侧紧贴文字按编译器文本模型垂直居中，Δ>3px warn）、
  `baseline_crossing`（hairline 基线落入柱体内部即 warn）。

**验证**：旧图例几何 Δ3.2px 被捕获；修复后全册静默；release 全量像素 PASS、release_eligible。
