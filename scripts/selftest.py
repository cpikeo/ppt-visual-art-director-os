...
def check_asset_workflow(work: pathlib.Path) -> None:
    ...
    check("assets: plan fingerprint is run-stable (volatile keys excluded)",
          stable_plan_sha(read_json(plan2)) == stable_plan_sha(read_json(plan)))
    spec, _ = vao.load_spec(spec_module(d / "base.py"))
    spec["asset_workflow"] = {"plan_sha256": stable_plan_sha(read_json(plan))}
    ...
    # v6.5 契约：prepare 时字节已存在 → 如实自动登记 existing（不补造生成历史），
    # 保留规划身份与 prompt/negative，QC 照常通过；不再制造「出图→阻断→补登记」回环。
    prepared = read_json(manifest_path)
    auto = prepared["assets"][0]
    existing_bytes, existing_code = vao._asset_qc_report(str(manifest_path), phase="release")
    check("assets: pre-existing bytes auto-register as existing at prepare (identity kept, QC passes)",
          existing_code == 0
          and auto["decision"] == "existing"
          and (auto.get("origin") or {}).get("kind") == "original"
          and bool((auto.get("origin") or {}).get("source"))
          and bool(auto.get("prompt")) and bool(auto.get("negative")),
          f"code={existing_code} decision={auto.get('decision')}")
...
def check_audit_fixes(work: pathlib.Path) -> None:
    ...
    def asset_case(name, second=False):
        ...
        spec["asset_workflow"] = {"plan_sha256":aw.stable_plan_sha(read(pp)), "plan_path":str(pp)}
    ...
    save(mp,manifest)
    assert vao._asset_qc_report(str(mp), phase="release")[1] == 0
    spec["asset_workflow"]={"plan_path":str(pp),"plan_sha256":aw.stable_plan_sha(bundle)}
...
def check_speed_profile(work: pathlib.Path) -> None:
    ...
    from asset_workflow import digest, stable_plan_sha, read_json
    ...
    spec_mod["asset_workflow"] = {"plan_sha256": stable_plan_sha(read_json(plan))}
